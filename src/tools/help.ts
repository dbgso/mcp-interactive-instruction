...
export function registerHelpTool(server: McpServer, markdownDir: string): void {
  ...
  server.registerTool(
    "help",
    {
      description: "List markdown files or get content by ID. Without arguments, lists all available documents with summaries. With an ID, returns the full content of that document.",
      inputSchema: {
        id: z
          .string()
          .optional()
